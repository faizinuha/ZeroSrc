...
SetupFileMissing=文件 %1 丢失。请修复问题或获取新的安装程序。
SetupFileCorrupt=安装文件已损坏。请获取新的安装程序。
SetupFileCorruptOrWrongVer=安装文件已损坏，或与此版本的安装程序不兼容。请修复问题或获取新的安装程序。
InvalidParameter=命令行传递了无效参数:%n%n%1
SetupAlreadyRunning=安装程序已在运行。
WindowsVersionNotSupported=此程序不支持您运行的 Windows 版本。
WindowsServicePackRequired=此程序需要 %1 Service Pack %2 或更高版本。
NotOnThisPlatform=此程序无法在 %1 下运行。
OnlyOnThisPlatform=此程序必须在 %1 下运行。
OnlyOnTheseArchitectures=此程序只能在运行以下 Windows 处理器架构的计算机上安装:%n%n%1
WinVersionTooLowError=此程序需要 %1 版本 %2 或更高。
WinVersionTooHighError=此程序无法在 %1 版本 %2 或更高下运行。
AdminPrivilegesRequired=安装此程序时您必须以管理员身份登录。
PowerUserPrivilegesRequired=安装此程序时您必须以管理员身份或高权限用户身份登录。
SetupAppRunningError=安装程序检测到 %1 正在运行。%n%n请关闭所有正在运行的实例，然后单击“确定”继续，或单击“取消”退出。
UninstallAppRunningError=卸载程序检测到 %1 正在运行。%n%n请关闭所有正在运行的实例，然后单击“确定”继续，或单击“取消”退出。
...
ExitSetupTitle=退出安装
ExitSetupMessage=安装尚未完成。如果您现在退出，程序将不会被安装。%n%n您可以稍后再次运行安装程序以完成安装。%n%n退出安装吗？
...
WelcomeLabel2=这将在您的计算机上安装 [name/ver]。%n%n在继续之前，建议您关闭所有其他应用程序。
...
DiskSpaceGBLabel=程序至少需要 [gb] GB 的磁盘空间。
DiskSpaceMBLabel=程序至少需要 [mb] MB 的磁盘空间。
...
InvalidDrive=您选择的驱动器或 UNC 共享不存在或无法访问。请选择其他文件夹。
DiskSpaceWarningTitle=磁盘空间不足
DiskSpaceWarning=安装程序至少需要 %1 KB 的可用空间，但所选驱动器只有 %2 KB 可用。%n%n您想继续吗？
DirNameTooLong=文件夹名称或路径太长。
InvalidDirName=文件夹名称无效。
BadDirName32=文件夹名称不能包含以下任何字符:%n%n%1
DirExistsTitle=文件夹已存在
DirExists=文件夹:%n%n%1%n%n已存在。您仍然想安装到该文件夹吗？
...
SelectComponentsLabel2=选择您想要安装的组件；清除您不想安装的组件。准备好继续时，请单击“下一步”。
FullInstallation=完整安装
CompactInstallation=精简安装
CustomInstallation=自定义安装
NoUninstallWarningTitle=组件已存在
NoUninstallWarning=安装程序检测到以下组件已安装在您的计算机上:%n%n%1%n%n取消选择这些组件将不会卸载它们。%n%n您想继续吗？
ComponentSize1=%1 KB
ComponentSize2=%1 MB
...
WizardReady=准备安装
ReadyLabel1=向导现在准备开始在您的计算机上安装 [name]。
ReadyLabel2a=单击“安装”继续安装，或单击“上一步”检查或更改任何设置。
...
PreparingDesc=向导正在准备在您的计算机上安装 [name]。
PreviousInstallNotCompleted=以前的程序安装/卸载尚未完成。您需要重启计算机以完成该过程。%n%n重启计算机后，请再次运行安装程序以完成 [name] 的安装。
CannotContinue=安装程序无法继续。请单击“取消”退出。
ApplicationsFound=以下应用程序正在使用安装程序需要更新的文件。建议您允许安装程序自动关闭这些应用程序。
ApplicationsFound2=以下应用程序正在使用安装程序需要更新的文件。建议您允许安装程序自动关闭这些应用程序。安装完成后，安装程序将尝试重启这些应用程序。
CloseApplications=自动关闭应用程序(&A)
DontCloseApplications=不关闭应用程序(&D)
ErrorCloseApplications=安装程序无法自动关闭所有应用程序。建议您在继续之前关闭所有正在使用安装程序需要更新的文件的应用程序。
PrepareToInstallNeedsRestart=安装程序需要重启您的计算机。重启计算机后，请再次运行安装程序以完成 [name] 的安装。%n%n您想现在重启吗？
...
FinishedLabelNoIcons=向导已在您的计算机上完成 [name] 的安装。
FinishedLabel=向导已在您的计算机上完成 [name] 的安装。可以通过选择安装的快捷方式来启动此应用程序。
...
YesRadio=是的，现在重启计算机(&Y)
NoRadio=不，我稍后重启计算机(&N)
...
SelectDiskLabel2=请插入磁盘 %1 并单击“确定”。%n%n如果此磁盘上的文件可以在下面显示的文件夹之外的其他文件夹中找到，请在下面输入正确的路径或单击“浏览”。
PathLabel=路径(&P):
FileNotInDir2=在“%2”中找不到文件“%1”。请插入正确的磁盘或选择其他文件夹。
SelectDirectoryLabel=请指定下一张磁盘的位置。

; *** Installation phase messages
SetupAborted=安装未完成。%n%n请纠正问题并再次运行安装。
...
VerificationKeyNotFound=在签名文件“%1”中未找到已知的密钥
...
UninstallUnknownEntry=在卸载日志中遇到了一个未知的条目 (%1)
ConfirmUninstall=您确定要完全删除 %1 及其所有组件吗？
UninstallOnlyOnWin64=此安装只能在 64 位 Windows 上卸载。
OnlyAdminCanUninstall=此卸载只能由具有管理员权限的用户执行。
UninstallStatusLabel=正在从您的计算机中删除 %1，请稍候。
UninstalledAll=%1 已成功从您的计算机中删除。
UninstalledMost=%1 卸载完成。%n%n部分元素无法被删除。这些您可以手动删除。
UninstalledAndNeedsRestart=为了完成 %1 的卸载，必须重启您的计算机。您想现在重启吗？
UninstallDataCorrupted=文件“%1”已损坏。无法卸载。

; *** Uninstallation phase messages
ConfirmDeleteSharedFileTitle=删除共享文件？
ConfirmDeleteSharedFile2=系统指示以下共享文件已不再被任何程序使用。您想让卸载程序删除这些共享文件吗？%n%n如果仍有程序使用这些文件而它们被删除了，那些程序可能无法正常工作。如果您不确定，请选择“否”。将文件留在系统中不会造成损害。
